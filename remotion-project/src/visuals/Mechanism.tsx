...
export const Mechanism: React.FC<Props> = ({ data: props }) => {
  ...
  return (
    <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", padding: "60px 100px" }}>
      <div style={{ textAlign: "center", marginBottom: 24, opacity: titleOpacity, transform: `translateY(${interpolate(titleP, [0, 1], [-16, 0])}px)` }}>
        <div style={{ fontSize: 38, fontWeight: 700, color: theme.gold, fontFamily: theme.font }}>{title}</div>
        {subtitle && <div style={{ fontSize: 20, color: theme.gray, fontFamily: theme.font, marginTop: 6 }}>{subtitle}</div>}
      </div>
            ...
            <React.Fragment key={i}>
              <div style={{
                ...
                <div style={{
                  ...
                  fontSize: 20, fontWeight: 900, color: isLast ? theme.tiffany : theme.grayLight, fontFamily: theme.font,
                }}>
                  {step.icon ?? (i + 1)}
                </div>
                {/* Content */}
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 22, fontWeight: 700, color: theme.white, fontFamily: theme.font }}>{step.title}</div>
                  <div style={{ fontSize: 18, color: theme.grayLight, fontFamily: theme.font, marginTop: 4, lineHeight: 1.5 }}>{step.explanation}</div>
                </div>
              </div>
              {!isLast && (
                <div style={{ textAlign: "center", fontSize: 22, color: `${theme.tiffany}40`, opacity: arrowOpacity }}>▼</div>
              )}
            </React.Fragment>
          );
        })}
