...
export const NewsTicker: React.FC<Props> = ({ data: props }) => {
  ...
  return (
    <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", justifyContent: "center", padding: "60px 100px" }}>
      <div style={{ fontSize: 46, fontWeight: 700, color: theme.gold, fontFamily: theme.font, textAlign: "center", marginBottom: 40, opacity: titleOpacity }}>
        {title}
      </div>
        ...
        {items.map((item, i) => {
          ...
          return (
            <div key={i} style={{
              ...
              <span style={{ fontSize: 32, color: typeColor, fontFamily: theme.font, flexShrink: 0 }}>{dot}</span>
              <span style={{ fontSize: 32, fontWeight: 600, color: theme.white, fontFamily: theme.font, flex: 1 }}>
                {item.headline}
              </span>
              {item.source && (
                <span style={{ fontSize: 28, color: theme.gray, fontFamily: theme.font, flexShrink: 0 }}>{item.source}</span>
              )}
            </div>
          );
        })}
