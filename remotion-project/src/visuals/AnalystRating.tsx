  ...
  return (
    <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", justifyContent: "center", padding: "60px 100px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24, opacity: titleOpacity, transform: `translateY(${interpolate(titleP, [0, 1], [-16, 0])}px)` }}>
        <div>
          <div style={{ fontSize: 44, fontWeight: 900, color: theme.white, fontFamily: theme.font }}>{data.name} ({data.ticker})</div>
          <div style={{ fontSize: 32, color: theme.grayLight, fontFamily: theme.font }}>현재가: {data.current_price}</div>
        </div>
        <div style={{ display: "flex", gap: 16 }}>
          <div style={{ padding: "8px 16px", borderRadius: 8, background: `${RATING_COLORS.buy}20`, border: `1px solid ${RATING_COLORS.buy}40` }}>
            <span style={{ fontSize: 30, fontWeight: 800, color: RATING_COLORS.buy, fontFamily: theme.font }}>매수 {buyCount}</span>
          </div>
          <div style={{ padding: "8px 16px", borderRadius: 8, background: `${RATING_COLORS.hold}20`, border: `1px solid ${RATING_COLORS.hold}40` }}>
            <span style={{ fontSize: 30, fontWeight: 800, color: RATING_COLORS.hold, fontFamily: theme.font }}>보유 {holdCount}</span>
          </div>
          <div style={{ padding: "8px 16px", borderRadius: 8, background: `${RATING_COLORS.sell}20`, border: `1px solid ${RATING_COLORS.sell}40` }}>
            <span style={{ fontSize: 30, fontWeight: 800, color: RATING_COLORS.sell, fontFamily: theme.font }}>매도 {sellCount}</span>
          </div>
        </div>
      </div>
        ...
        {data.analysts.map((a, i) => {
          ...
          return (
            <div key={i} style={{
              ...
              <div style={{ width: 280, fontSize: 32, fontWeight: 700, color: theme.white, fontFamily: theme.font, whiteSpace: "nowrap" }}>{a.firm}</div>
              <div style={{ width: 90, fontSize: 28, fontWeight: 800, color, fontFamily: theme.font, padding: "6px 14px", background: `${color}15`, borderRadius: 6, textAlign: "center" }}>
                {RATING_LABELS[a.rating]}
              </div>
              <div style={{ fontSize: 30, fontWeight: 800, color: theme.tiffany, fontFamily: theme.font }}>→ {a.target}</div>
              {a.date && <div style={{ fontSize: 26, color: theme.gray, fontFamily: theme.font, marginLeft: "auto" }}>{a.date}</div>}
            </div>
          );
        })}
      </div>
      ...
      {data.consensus && (
        <div style={{ fontSize: 32, color: theme.tiffany, fontFamily: theme.font, textAlign: "center", marginTop: 16, fontWeight: 600, opacity: interpolate(frame, [40, 52], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }) }}>
          컨센서스: {data.consensus}
        </div>
      )}
    </div>
