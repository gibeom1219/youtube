  ...
  return (
    <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", justifyContent: "center", padding: "60px 100px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24, opacity: titleOpacity, transform: `translateY(${interpolate(titleP, [0, 1], [-16, 0])}px)` }}>
        <div>
          <div style={{ fontSize: 44, fontWeight: 900, color: theme.white, fontFamily: theme.font, textShadow: theme.textShadow.medium }}>{data.name} ({data.ticker})</div>
          <div style={{ fontSize: 32, color: theme.grayLight, fontFamily: theme.fontNum, textShadow: theme.textShadow.medium }}>현재가: {data.current_price}</div>
        </div>
        <div style={{ display: "flex", gap: 16 }}>
          <div style={{ padding: "8px 16px", borderRadius: 8, background: `${RATING_COLORS.buy}20`, border: `1px solid ${RATING_COLORS.buy}40` }}>
            <span style={{ fontSize: 30, fontWeight: 800, color: RATING_COLORS.buy, fontFamily: theme.fontNum, textShadow: theme.textShadow.medium }}>매수 {buyCount}</span>
          </div>
          <div style={{ padding: "8px 16px", borderRadius: 8, background: `${RATING_COLORS.hold}20`, border: `1px solid ${RATING_COLORS.hold}40` }}>
            <span style={{ fontSize: 30, fontWeight: 800, color: RATING_COLORS.hold, fontFamily: theme.fontNum, textShadow: theme.textShadow.medium }}>보유 {holdCount}</span>
          </div>
          <div style={{ padding: "8px 16px", borderRadius: 8, background: `${RATING_COLORS.sell}20`, border: `1px solid ${RATING_COLORS.sell}40` }}>
            <span style={{ fontSize: 30, fontWeight: 800, color: RATING_COLORS.sell, fontFamily: theme.fontNum, textShadow: theme.textShadow.medium }}>매도 {sellCount}</span>
          </div>
        </div>
      </div>
        ...
        {(data.analysts ?? []).map((a, i) => {
          ...
          return (
            <div key={i} style={{
              ...
              <div style={{ width: 280, fontSize: 32, fontWeight: 700, color: theme.white, fontFamily: theme.font, whiteSpace: "nowrap", textShadow: theme.textShadow.medium }}>{a.firm}</div>
              <div style={{ width: 90, fontSize: 28, fontWeight: 800, color, fontFamily: theme.font, padding: "6px 14px", background: `${color}30`, borderRadius: 6, textAlign: "center", textShadow: theme.textShadow.medium }}>
                {RATING_LABELS[a.rating]}
              </div>
              <div style={{ fontSize: 30, fontWeight: 800, color: theme.tiffany, fontFamily: theme.fontNum, textShadow: theme.textShadow.medium }}>→ {a.target}</div>
              {a.date && <div style={{ fontSize: 26, color: theme.gray, fontFamily: theme.font, marginLeft: "auto", textShadow: theme.textShadow.medium }}>{a.date}</div>}
            </div>
          );
        })}
      </div>
      ...
      {data.consensus && (
        <div style={{ fontSize: 32, color: theme.tiffany, fontFamily: theme.font, textAlign: "center", marginTop: 16, fontWeight: 600, textShadow: theme.textShadow.medium, opacity: interpolate(frame, [40, 52], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }) }}>
          컨센서스: {data.consensus}
        </div>
      )}
    </div>
