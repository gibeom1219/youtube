...
export const DarkRadar: React.FC<Props> = ({ data }) => {
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 30 }}>
          <svg width={380} height={380} viewBox="0 0 380 380" style={{ flexShrink: 0 }}>
            {gridLevels.map((level, li) => {
              ...
              return (
                <path key={li} d={gridPath} fill="none" stroke="rgba(255,255,255,0.1)" strokeWidth={1}
            ...
            {dataPoints.map((p, i) => (
              <circle key={i} cx={p.x} cy={p.y} r={5} fill="#060d0c" stroke={color} strokeWidth={2.5}
            ...
            {axes.map((axis, i) => {
              ...
              return (
                <text key={i} x={p.x} y={p.y + 5} textAnchor="middle" fill="rgba(255,255,255,0.7)" fontSize={22} fontWeight={700}
            ...
            {axes.map((axis, i) => {
              ...
              return (
                <div key={i} style={{
                  ...
                  <span style={{ fontSize: 28, fontWeight: 600, color: "rgba(255,255,255,0.8)", fontFamily: theme.font }}>
                    {axis.label}
                  </span>
                  <span style={{ fontSize: 28, fontWeight: 900, color: "#fff", fontFamily: theme.font, marginLeft: "auto" }}>
                    {axis.value}
                  </span>
                </div>
              );
