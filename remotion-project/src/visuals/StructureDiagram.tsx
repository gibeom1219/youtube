...
export const StructureDiagram: React.FC<Props> = ({ data: props }) => {
  ...
  return (
    <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", padding: "50px 80px" }}>
      <div style={{ fontSize: 38, fontWeight: 700, color: theme.gold, fontFamily: theme.font, textAlign: "center", marginBottom: 10, opacity: titleOpacity, transform: `translateY(${interpolate(titleP, [0, 1], [-16, 0])}px)` }}>
        {props.title}
      </div>
      {props.description && (
      ...
      <div style={{ flex: 1, position: "relative" }}>
          ...
          {(props.connections ?? []).map((conn, i) => {
            ...
            return (
              <g key={i} opacity={connOpacity}>
                ...
                {conn.label && (
                  <text x={(fromPos.x + toPos.x) / 2} y={(fromPos.y + toPos.y) / 2 - 10} fill={theme.gray} fontSize={16} fontFamily={theme.font} textAnchor="middle">
                    {conn.label}
                  </text>
                )}
              </g>
        ...
        {props.elements.map((el, i) => {
          ...
          return (
            <div key={i} style={{
              position: "absolute", left: pos.x - (isCenter ? 100 : 85), top: pos.y - (isCenter ? 55 : 40),
              width: isCenter ? 200 : 170, padding: isCenter ? "20px 16px" : "14px 14px",
