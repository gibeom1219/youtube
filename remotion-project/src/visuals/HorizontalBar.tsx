...
export const HorizontalBar: React.FC<Props> = ({ data: props }) => {
  ...
  return (
    <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", justifyContent: "center", padding: "60px 120px" }}>
      <div style={{ fontSize: 46, fontWeight: 700, color: theme.gold, fontFamily: theme.font, textAlign: "center", marginBottom: 40, textShadow: theme.textShadow.medium, opacity: titleOpacity, transform: `translateY(${interpolate(titleP, [0, 1], [-16, 0])}px)` }}>
        {title}
      </div>
        ...
        {items.map((item, i) => {
          ...
          return (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 16, opacity: rowOpacity }}>
              <div style={{ width: 240, fontSize: 34, fontWeight: 600, color: theme.white, fontFamily: theme.font, textAlign: "right", flexShrink: 0, whiteSpace: "nowrap", textShadow: theme.textShadow.medium }}>
                {item.label}
              </div>
              <div style={{ flex: 1, height: 44, background: "rgba(255,255,255,0.20)", borderRadius: 10, overflow: "hidden" }}>
                <div style={{
              ...
              <div style={{ width: 160, fontSize: 32, fontWeight: 800, color, fontFamily: theme.fontNum, textAlign: "right", flexShrink: 0, whiteSpace: "nowrap", textShadow: theme.textShadow.medium }}>
                {item.value}{unit}
              </div>
            </div>
          );
