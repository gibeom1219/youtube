...
interface Props {
  data: { title: string; buy_pct: number; sell_pct: number; neutral_pct?: number; buy_label?: string; sell_label?: string; description?: string };
}
...
export const SentimentBar: React.FC<Props> = ({ data }) => {
  ...
  const neutral = data.neutral_pct ?? (100 - data.buy_pct - data.sell_pct);
  ...
  const winner = data.buy_pct > data.sell_pct ? "buy" : data.sell_pct > data.buy_pct ? "sell" : "neutral";
  ...
  return (
    <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "80px 120px", gap: 30 }}>
      ...
      {/* Bar */}
      <div style={{ width: "100%", maxWidth: 1000, height: 56, borderRadius: 28, overflow: "hidden", display: "flex", opacity: barOpacity }}>
        <div style={{ width: `${data.buy_pct * barP}%`, height: "100%", background: `linear-gradient(90deg, ${theme.green}90, ${theme.green})` }} />
        {neutral > 0 && <div style={{ width: `${neutral * barP}%`, height: "100%", background: "rgba(255,255,255,0.08)" }} />}
        <div style={{ width: `${data.sell_pct * barP}%`, height: "100%", background: `linear-gradient(90deg, ${theme.red}, ${theme.red}90)` }} />
      </div>

      {/* Percentages */}
      <div style={{ display: "flex", justifyContent: "space-between", width: "100%", maxWidth: 1000, opacity: numOpacity }}>
        <span style={{ fontSize: 48, fontWeight: 900, color: winner === "buy" ? theme.green : theme.grayLight, fontFamily: theme.font }}>{data.buy_pct}%</span>
        {neutral > 0 && <span style={{ fontSize: 32, fontWeight: 600, color: theme.gray, fontFamily: theme.font }}>{neutral}%</span>}
        <span style={{ fontSize: 48, fontWeight: 900, color: winner === "sell" ? theme.red : theme.grayLight, fontFamily: theme.font }}>{data.sell_pct}%</span>
      </div>

      {data.description && (
        <div style={{ fontSize: 24, color: theme.grayLight, fontFamily: theme.font, textAlign: "center", maxWidth: 900, opacity: descOpacity, marginTop: 10 }}>
          {data.description}
        </div>
      )}
    </div>
