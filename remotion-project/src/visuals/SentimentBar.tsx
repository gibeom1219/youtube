...
export const SentimentBar: React.FC<Props> = ({ data }) => {
  ...
  return (
    <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "80px 120px", gap: 30 }}>
      <div style={{ fontSize: 40, fontWeight: 700, color: theme.gold, fontFamily: theme.font, textShadow: theme.textShadow.medium, opacity: titleOpacity, transform: `translateY(${interpolate(titleP, [0, 1], [-16, 0])}px)` }}>
        {data.title}
      </div>

      {/* Labels */}
      <div style={{ display: "flex", justifyContent: "space-between", width: "100%", maxWidth: 1000, opacity: barOpacity }}>
        <span style={{ fontSize: 34, fontWeight: 800, color: theme.green, fontFamily: theme.font, textShadow: theme.textShadow.medium }}>{buyLabel}</span>
        {neutral > 0 && <span style={{ fontSize: 34, color: theme.gray, fontFamily: theme.font, textShadow: theme.textShadow.medium }}>중립</span>}
        <span style={{ fontSize: 34, fontWeight: 800, color: theme.red, fontFamily: theme.font, textShadow: theme.textShadow.medium }}>{sellLabel}</span>
      </div>
      ...
      <div style={{ width: "100%", maxWidth: 1000, height: 72, borderRadius: 36, overflow: "hidden", display: "flex", opacity: barOpacity }}>
        ...
        {neutral > 0 && <div style={{ width: `${neutral * barP}%`, height: "100%", background: "rgba(255,255,255,0.22)" }} />}
      ...
      <div style={{ display: "flex", justifyContent: "space-between", width: "100%", maxWidth: 1000, opacity: numOpacity }}>
        <span style={{ fontSize: 64, fontWeight: 900, color: winner === "buy" ? theme.green : theme.grayLight, fontFamily: theme.fontNum, textShadow: theme.textShadow.medium }}>{buyPct}%</span>
        {neutral > 0 && <span style={{ fontSize: 44, fontWeight: 600, color: theme.gray, fontFamily: theme.fontNum, textShadow: theme.textShadow.medium }}>{neutral}%</span>}
        <span style={{ fontSize: 64, fontWeight: 900, color: winner === "sell" ? theme.red : theme.grayLight, fontFamily: theme.fontNum, textShadow: theme.textShadow.medium }}>{sellPct}%</span>
      </div>

      {data.description && (
        <div style={{ fontSize: 30, color: theme.grayLight, fontFamily: theme.font, textAlign: "center", maxWidth: 900, opacity: descOpacity, marginTop: 10, textShadow: theme.textShadow.medium }}>
          {data.description}
        </div>
      )}
    </div>
