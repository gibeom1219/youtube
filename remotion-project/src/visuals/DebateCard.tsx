  ...
  return (
    <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", justifyContent: "center", padding: "60px 100px", gap: 20 }}>
      ...
      <div style={{
        ...
        background: `${theme.tiffany}55`, border: `1px solid ${theme.tiffany}60`,
        opacity: quoteOpacity, transform: `translateY(${interpolate(quoteP, [0, 1], [-20, 0])}px)`,
      }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 16 }}>
          <div style={{ fontSize: 34, fontWeight: 800, color: theme.tiffany, fontFamily: theme.font, textShadow: theme.textShadow.medium }}>{data.speaker}</div>
          {data.role && <div style={{ fontSize: 30, color: theme.gray, fontFamily: theme.font, textShadow: theme.textShadow.medium }}>{data.role}</div>}
        </div>
        <div style={{ fontSize: 36, color: theme.white, fontFamily: theme.fontQuote, lineHeight: 1.5, fontWeight: 500, textShadow: theme.textShadow.medium }}>
          "{data.quote}"
        </div>
      </div>
        ...
        {(data.reactions ?? []).map((r, i) => {
          ...
          return (
            <div key={i} style={{
              ...
              background: `${color}50`, borderLeft: `4px solid ${color}80`,
              ...
              <span style={{ fontSize: 32, color: theme.white, fontFamily: theme.font, lineHeight: 1.5, textShadow: theme.textShadow.medium }}>{r.text}</span>
            </div>
          );
        })}
      </div>
