  ...
  return (
    <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", padding: "70px 120px" }}>
      <div style={{
        ...
        background: "rgba(129,216,208,0.25)", border: "1px solid rgba(129,216,208,0.22)",
      }}>
        {/* Avatar */}
        <div style={{
          ...
          background: `${theme.tiffany}25`, borderRadius: "50%", border: `2px solid ${theme.tiffany}30`,
        }}>
          {data.avatar}
        </div>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ opacity: infoOpacity, transform: `translateX(${interpolate(infoP, [0, 1], [20, 0])}px)` }}>
            <div style={{ fontSize: 52, fontWeight: 900, color: theme.white, fontFamily: theme.font, textShadow: theme.textShadow.medium }}>{data.name}</div>
            <div style={{ fontSize: 30, color: theme.tiffany, fontFamily: theme.font, fontWeight: 600, marginTop: 6, textShadow: theme.textShadow.medium }}>{data.role}</div>
            <div style={{ fontSize: 26, color: theme.white, fontFamily: theme.font, fontWeight: 500 }}>{data.organization}</div>
          </div>

          <div style={{
            fontSize: 28, color: theme.white, fontFamily: theme.font, lineHeight: 1.6, fontWeight: 500,
            textShadow: theme.textShadow.light,
            opacity: bioOpacity, marginTop: 8,
            paddingTop: 16, borderTop: `1px solid rgba(129,216,208,0.25)`,
          }}>
            {data.bio}
          </div>
            ...
            <div style={{ display: "flex", gap: 32, marginTop: 12, opacity: statsOpacity }}>
              {(data.stats ?? []).map((s, i) => (
                <div key={i} style={{ textAlign: "center" }}>
                  ...
                  <div style={{ fontSize: 26, color: theme.grayLight, fontFamily: theme.font, fontWeight: 600 }}>{s.label}</div>
                </div>
              ))}
            </div>
          )}
