...
각 장면은 **팩트 → 내 해석 → 내 생각** 흐름으로 작성:

```
❌ 나쁜 예: "M2 통화량이 41조원 늘었습니다."
✅ 좋은 예: "M2 통화량이 10월에만 41조원이 늘었대요. 7개월 연속 증가세라고 합니다.
            사실 이미 오래 전부터 한국 정부는 만성 적자 상태죠."
```

나레이션에 자연스럽게 녹여야 할 표현들:
- "근데 여기서 문제가 뭐냐면은", "한마디로 요약하면은"
- "이거를 어떻게 봐야 되냐면", "사실 관계만 봤을 때는"
- "아시겠지만", "많은 분들이 놓치시는 부분인데"
- "솔직히 말씀드리면", "제 눈엔 이렇게 보여요"
- "결국은 이거라고 생각해요", "시장이 좀 과하게 반응하는 거 같아요"

**하나의 주제를 여러 씬에 걸쳐 깊이 있게 풀어갈 것**
- 씬이 바뀌어도 이야기 흐름이 자연스럽게 이어져야 함
- 이전 씬과 연결: "자, 그럼 이제~", "근데 여기서 하나 더~", "다음으로~"
- 한 포인트를 구체적인 숫자·사례·비유와 함께 충분히 설명
